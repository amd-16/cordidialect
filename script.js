...
/* ══════════════════════════════════════════════════
   AUDIO ENGINE — Recorded audio (primary) + TTS (fallback)
   ══════════════════════════════════════════════════ */
...
function ttsSpeak(word, btn) {
  if (!isPremium()) { 
    openPremiumModal('Audio Pronunciation'); 
    return 
  }
  ...
  // Generate ALL possible naming styles
  function buildVariants(w) {
    const base = clean(w).toLowerCase()

    return [
      base,                         // man-esol / ma-ud
      base.replace(/-/g, ' '),     // man esol
      base.replace(/\s+/g, '-'),   // man-esol
      base.replace(/-/g, ''),      // manesol
      base.replace(/\s+/g, '')     // manesol
    ]
  }

  const variants = [...new Set(buildVariants(word))]
  const tryPaths = []

  // Build full path list
  variants.forEach(v => {
    tryExts.forEach(ext => {
      tryPaths.push(`pronunciations/${v}.${ext}`)
    })
  })
  ...
  function tryNextPath(index) {
    ...
    const audio = new Audio(tryPaths[index])
    currentAudio = audio
